...
project = "Octopus"
copyright = "2024-2026 Merck KGaA, Darmstadt, Germany and/or its affiliates. All rights reserved."  # noqa
...
output_dir = __location__ / "sdk"
octopus_module_dir = __location__.parent / "octopus"
...
try:
    ...
    apidoc.main(
        [
            *args,
            str(octopus_module_dir),
            str(octopus_module_dir / "__init__.py"),
        ]
    )
...
nitpick_ignore_regex = [
    # Ignore everything that does not include octo
    (r"py:.*", r"^(?!.*octo).*"),
...
linkcheck_ignore = [r"https://github.com/emdgroup/octopus/issues/"]
...
def autodoc_process_docstring(app, what, name, obj, options, lines):
    ...
    if "__init__" in name and len(lines) > 0:
        lines.append("For details on the parameters, see **Public attributes and properties**.")
