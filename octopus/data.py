...
class OctoData:
    ...
    def check_nans_infs(self) -> bool:
        """Check if all relevant columns are free of NaNs or Infs."""
        pd.options.mode.use_inf_as_na = True  # also find np.inf
    ...
    def __attrs_post_init__(self):
        ...
        if self.check_rowid_unique():
            raise ValueError("Row_ID column is not unique")
        if self.check_columns_exist():
            raise ValueError("Defined columns missing in dataframe")
        if self.check_shared_columns():
            raise ValueError("Shared columns in properties")
        if self.check_duplicates_features():
            print("Warning: Duplicates (rows) in features")
            if self.datasplit_type == "sample":
                raise ValueError("Feature duplicates require different datasplit type")
        ...
        if self.check_nans_infs():
            raise ValueError("NaNs or Infs in dataframe")
        if self.check_nonnumeric():
            raise ValueError("Non-numeric relevant columns in dataframe")
