...
class OctoDataHealthChecker:
    ...
    def _check_critical_column_missing_values(self):
        ...
        critical_missing = [
            col for col in critical_columns if missing_value_share_col.get(col, 0) > 0
        ]
        if critical_missing:
            self.add_issue(
                category="columns",
                ...
                severity="Critical",
                description=(
                    "These critical columns (target, sample_id, or row_id) have "
                    "missing values."
                ),
                action=(
                    "Investigate and resolve missing values in these columns "
                    "immediately. These are crucial for model training "
                    "and data integrity."
                ),
            )
    ...
    def _check_feature_column_missing_values(self):
        ...
        high_missing_cols = [
            ...
            if missing_value_share_col.get(col, 0) > 0.25
        ]
        low_missing_cols = [
            col
            for col in self.feature_columns
            if 0 < missing_value_share_col.get(col, 0) <= 0.25
        ]
        ...
        if high_missing_cols:
            self.add_issue(
                ...
                action=(
                    "Consider removing these columns or using advanced imputation "
                    "techniques."
                ),
            )
    ...
    def _check_row_missing_values(self):
        ...
        low_missing_rows = missing_value_share_row[
            (0 < missing_value_share_row) & (missing_value_share_row <= 0.5)
        ]
        ...
        if not high_missing_rows.empty:
            self.add_issue(
                ...
                severity="high",
                description=(
                    f"{len(high_missing_rows)} rows have more than 10% missing values."
                ),
                action=(
                    "Consider removing these rows or using advanced imputation "
                    "techniques."
                ),
            )
        ...
        if not low_missing_rows.empty:
            self.add_issue(
                ...
                severity="Info",
                description=(
                    f"{len(low_missing_rows)} rows have some missing values (<=10%)."
                ),
    ...
    def _check_int_col_with_few_uniques(self, threshold: int = 5):
        ...
        int_cols_with_few_uniques = {
            ...
            and 2 < self.data[col].nunique() <= threshold
        }
        ...
        if int_cols_with_few_uniques:
            ...
            self.add_issue(
                ...
                severity="Warning",
                description=(
                    "These integer columns have between 3 and "
                    f"{threshold} unique values: {', '.join(affected_items)}"
                ),
                action=(
                    "Consider converting these columns to categorical variables "
                    "or investigate if they should be ordinal features."
                ),
            )
    ...
    def _check_duplicated_features(self):
        ...
        if self.sample_id is not None:
            duplicated_features_and_sample = (
                self.data[self.feature_columns + [self.sample_id]].duplicated().any()
            )
        else:
        ...
        if duplicated_features:
            self.add_issue(
                ...
                severity="Warning",
                description=(
                    "There are duplicated rows when considering all feature columns."
                ),
                action=(
                    "Investigate the cause of these duplicates and consider removing "
        ...
        if duplicated_features_and_sample:
            self.add_issue(
                ...
                severity="Critical",
                description=(
                    "There are duplicated rows when considering all feature "
                    "columns and the sample ID."
                ),
                action=(
                    "This is a critical issue. Investigate and resolve these "
                    "duplicates immediately as they may indicate data integrity "
                    "problems."
                ),
            )

    def _check_feature_feature_correlation(
        self, method: str = "pearson", threshold: float = 0.8
    ):
        """Find columns in the DataFrame that are highly correlated."""
        # Filter only numeric columns
        numeric_features = (
            self.data[self.feature_columns].select_dtypes(include=[float, int]).columns
        )
        ...
        for group in merged_groups:
            ...
            self.add_issue(
                ...
                description=(
                    ...
                    f"(>{threshold}): {correlation_description}"
                ),
                action=(
    ...
    def _check_identical_features(self):
        ...
        for feature, identical_list in identical_features.items():
            self.add_issue(
                ...
                severity="Warning",
                description=(
                    f"The feature '{feature}' is identical to the following "
                    "feature(s): {', '.join(identical_list)}"
                ),
                action=(
                    "Consider removing redundant features to simplify the dataset "
    ...
    def _check_duplicated_rows(self):
        ...
        if not duplicated_rows.empty:
            ...
            self.add_issue(
                ...
                action=(
                    "Investigate these duplicates and consider removing or "
                    "consolidating them based on your data requirements."
                ),
            )
    ...
    def _check_infinity_values(self):
        ...
        numeric_df = self.data[self.feature_columns].apply(
    ...
    def _check_string_mismatch(self):
        ...
        for column in self.feature_columns:
            if (
                ...
                try:
                    ...
                    column_values = (
                        self.data[column].dropna().apply(remove_numbers).unique()
                    )
        ...
        if string_mismatch:
            for column, similar_groups in string_mismatch.items():
                description = (
                    f"Column '{column}' contains similar strings that might "
                    "be misspellings or inconsistencies:\n"
                )
                ...
                self.add_issue(
                    ...
                    description=description.strip(),
                    action=(
                        "Review these similar strings and consider standardizing "
                        "them to improve data consistency."
                    ),
                )
    ...
    def _check_string_out_of_bounds(self, length_threshold_factor=2):
        ...
        for column in self.feature_columns:
            if (
                ...
                try:
                    column_values = (
                        self.data[column].dropna().tolist()
                    )  # Drop NaN values and convert to list

                    # Calculate the average length of strings in the column
                    avg_length = sum(len(str(value)) for value in column_values) / len(
                        column_values
                    )
                    ...
                    long_strings = [
                        value
                        for value in column_values
                        if len(str(value)) > length_threshold_factor * avg_length
                    ]
                    if long_strings:
        ...
        if long_string:
            for column, long_strings in long_string.items():
                description = (
                    f"Column '{column}' contains strings that are significantly "
                ...
                self.add_issue(
                    ...
                    description=description.strip(),
                    action=(
                        "Review these unusually long strings and consider if they "
                        "are valid or if they need cleaning or truncation."
                    ),
                )
