...
from octopus.manager.ray_parallel import init_ray
...
class WorkflowTaskRunner:
    ...
    Args:
        ...
        resources: Resource configuration for CPU allocation.
        log_dir: Directory for individual worker logs.
    """

    def __init__(self, workflow: list[Task], resources: "ResourceConfig", log_dir: UPath):
    ...
    def _ensure_ray_connected(self) -> None:
        """Ensure Ray is connected (safety check).

        Ray should already be initialized by OctoManager.run_outer_experiments().
        This is a defensive check in case WorkflowTaskRunner is used independently.
        """
    ...
    def _create_experiment(self, base_experiment: OctoExperiment, task: Task) -> OctoExperiment:
        ...
        experiment.num_assigned_cpus = self.resources.cpus_per_experiment
        return experiment
    ...
    def _load_experiment(self, base_experiment: OctoExperiment, task: Task) -> OctoExperiment:
        ...
        experiment = OctoExperiment.from_pickle(load_path)
        logger.info(f"Loaded existing experiment from: {load_path}")
        return experiment
