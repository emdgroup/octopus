...
def add_pooling_scores(pool, scores, target_metric, target_assignments):
    ...
    if target_metric in ["AUCROC", "LOGLOSS"]:
        for part in pool.keys():
            ...
            scores[part + "_pool_soft"] = metrics_inventory[target_metric](
                target, probabilities
            )
            scores[part + "_pool_hard"] = metrics_inventory[target_metric](
                target, predictions
            )
        ...
        for part in pool.keys():
            ...
            ci, _, _, _, _ = metrics_inventory[target_metric](
